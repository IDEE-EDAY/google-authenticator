...
function loginform() {
    echo "\t<p>\n";
	echo "\t\t<label title=\"".__('If you don\'t have Authenticator enabled for your WordPress account, leave this field empty.','google-authenticator')."\">".__('Authenticator digits','google-authenticator')."<span id=\"google-auth-info\"></span><br />\n";
	...
	if ( isset( $user->ID ) && trim(get_user_option( 'googleauthenticator_enabled', $user->ID ) ) == 'enabled' ) {
		...
		if ( $timeslot = $this->verify( $GA_secret, $otp, $GA_relaxedmode, $lasttimeslot ) ) {
			...
			if ( trim( get_user_option( 'googleauthenticator_pwdenabled', $user->ID ) ) == 'enabled' && ( defined('XMLRPC_REQUEST') || defined('APP_REQUEST') ) ) {
				...
				return new WP_Error( 'invalid_google_authenticator_token', __( '<strong>ERROR</strong>: The Authenticator digits are incorrect or have expired.', 'google-authenticator' ) );
			}	
		}
	}
